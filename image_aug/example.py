...
class Augumentor():
    ...
    def fit(self):
        ...
        for annotation in annotations:
            key = annotation[0].split(',')[0]#image name
            value = np.array([annotation[1:]])#other feature
            if key in total_boxes.keys():
                total_boxes[key] = np.concatenate((total_boxes[key],value),axis=0)
            else:
                total_boxes[key] = value
            #key = annotation[0].split(' ')[0]#image name
            #value = np.array([annotation[0].split(' ')[1:]])#other feature
            #if key in total_boxes.keys():
                #total_boxes[key] = np.concatenate((total_boxes[key],value),axis=0)
            #else:
                #total_boxes[key] = value  
...
if __name__ == "__main__":
    ...
    if not os.path.exists(config.augmented_csv_file):
        os.mknod(config.augmented_csv_file)        
